...
namespace BulgarianDestinations.Tests.CartTests
{
    ...
    public class OrderArticulsTest
    {
        ...
        public void TestInitialize()
        {
            ...
            articulOrders = new List<ArticulOrder>();



            var options = new DbContextOptionsBuilder<ApplicationDbContext>()
                    .UseInMemoryDatabase(databaseName: "OrderArticulsTestInMemoryDb") // Give a Unique name to the DB
                    .Options;
            ...
            dbContext.AddRange(articulOrders);
        ...
        [Test]
        public void Test_OrderArticulsTest()
        {
            service.OrderArticuls(1);

            int actualCount = repository.AllReadOnly<Order>().Count();
            int expectedCount = 1;

            int actualCountInAO = repository.AllReadOnly<ArticulOrder>().Count();
            int expectedCountInAO = 2;

            Assert.That(actualCount, Is.EqualTo(expectedCount));
            Assert.That(actualCountInAO, Is.EqualTo(expectedCountInAO));
        }
    }
}
